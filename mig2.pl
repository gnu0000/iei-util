...
sub XmlData
   ...
   my $chargecount = ref($charge) =~ /HASH/i ? 1 : ref($charge) =~ /ARRAY/i ? scalar @{$charge} : 0;
   my ($arrangement) = $doc->{Invoice}->{InvoiceAccountInfo}->{Account}->{Standing} =~ /PaymentArrangement/i;
   ...
   return 
      ...
      $ias->{BalanceForward}->{Amount},                                  #  BalanceForward
      $ias->{CurrentCharges}->{Amount},                                  #  CurrentCharges
      $ias->{TotalAmountDue}->{Amount},                                  #  TotalDue
      $doc->{Invoice}->{ConstituentAccounts}->{Account}->{Usage},        #  Therms
